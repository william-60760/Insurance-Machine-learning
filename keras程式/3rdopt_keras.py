...
# 只看數值欄位的相關係數
corr = train2_df.corr(numeric_only=True)["Premium Amount"]
# 把 Premium Amount 自己拿掉，只看其他欄位和它的相關
corr = corr.drop(labels=["Premium Amount"])
# 用絕對值排序，相關程度由大到小
corr_sorted = corr.abs().sort_values(ascending=False)
print(corr_sorted)

top_features = corr_sorted.head(6).index.tolist()
print("第三次優化選用的特徵：", top_features)

X = train2_df[top_features].values
Y = train2_df["Premium Amount"].values
...
X_test3 = test2_train2_df[top_features].values
Y_test3 = model.predict(X_test3, verbose=0)

submission3_train2_df = pd.DataFrame({"id": np.arange(1200000, 1200000 + len(Y_test3)), "Premium Amount": Y_test3.flatten()})
submission3_train2_df.to_csv("bfkeras_submission3.csv", index=False)
