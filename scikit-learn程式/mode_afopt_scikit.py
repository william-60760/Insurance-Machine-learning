...
df_train = pd.read_csv("mode_afopt1_train_cleaned.csv")
#df_train.info() #檢查資料用
...
def insurance_model(model, data, predictors, outcome, t_size, name): #定義函式，輸入模型、資料、用於預測的變數、結果變數、測試資料比例
    ...
    joblib.dump(model,f"Insurance-{name}-mode.afopt1.pkl", compress=3) #匯出模型，檔名包含模型名稱
...
predictor_var = ["Age","Annual Income","Number of Dependents","Health Score",
                 "Previous Claims","Vehicle Age","Credit Score","Insurance Duration",
                 "Occupation Number","Customer Feedback Number","Gender Number",
                 "Marital Status Number","Location Number","Policy Type Number","Education Level Number",
                 "Smoking Status Number","Property Type Number"]
